...
void sortnum(struct datavalues d,const int col,const int row,const int roworcol,int med){
    ...
    //             for (int j = 0; j<d.totalcol;j++)
    //             {
    //                 if(d.fulldata[row][j] < d.fulldata[row][minipos])
    //                 {
    //                     ar[i][j]=d.fulldata[row][j];
    //                 }
    //                 else
    //                 ar[i][j]=d.fulldata[row][j];
    //             }
    ...
    //         {
    //             if(d.fulldata[col][j] < d.fulldata[col][minipos]){
    //             ar[i][j]=d.fulldata[col][j];
    //             }
    //         }
    //     }
    // }
    ...
    for(int i=0; i<d.totalrow;i++)
    {
        if(d.computablecols==1)
        {
            for(int j=0; j<d.totalcol;j++)
            {
                ar[i][j]=d.fulldata[i][j];
            }
        }
    }
      int t= sizeof(damage)/sizeof(damage[0]);
        sort(damage, damage+t, greater<int>());
    cout<<"------------------------\n";
