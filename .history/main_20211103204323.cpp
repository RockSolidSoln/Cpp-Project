...
vec loadfile();
void clearfile();
int** database(string,string[],int,int);
void savefile();
...
void clearfile() 
{
    ...
    database(arr2,filename,arr1,row,column);
    for(int i=0;i<row;i++) {cout<<arr2[i][0];}
}

//---------------------------------------------Ahmad Ayaan------------------------------------------------------
// -----------------------------This function stores the data base file in the vector------------------------------------
int database(int **arr2, string filename, string arr1[], int row, int column )
{   
