...
void deleteuser() 
{
    ...
    {   
        if (get<0>(userdata)==name,get<2>(userdata)==pass)
             {  
                cout << get<0>(userdata) << ": "
                 << ((get<1>(userdata)==1)?"admin":"user")
                 << ": " <<get<2>(userdata) << ": "
                 << ((get<3>(userdata)==1)?"Deleted":"Active")
                 << endl;
    ...
    for(int i=0;i<users.size();i++)
    {
        cout<<get<0>(userdata)<<" "<<get<1>(userdata)<<" "<<get<2>(userdata)<<" "<<get<3>(userdata)<<endl;
    }
    outfile.close();
